...
diff_compare_data[["kappa_values"]] %>% hist(breaks=10)
...
#### SAMPLES ####
diff_pooled_samples <- sampling(pooled_student_t_oup,
                                diff_compare_data,
                                iter=iter,
                                chains=chains,
                                init=1)

diff_non_pooled_samples <- sampling(non_pooled_student_t_oup,
                                    diff_compare_data,
                                    iter=iter,
                                    chains=chains, 
                                    init=1)

diff_partially_pooled_samples <- sampling(hierarchical_student_t_oup,
                                          diff_compare_data,
                                          iter=iter,
                                          chains=chains,
                                          init=1)
...
diff_compare_samples_list <- list(pooled_samples = diff_pooled_samples,
                                  non_pooled_samples = diff_non_pooled_samples, 
                                  partially_pooled_samples = diff_partially_pooled_samples)
...
for(par in c("lambda", "sigma", "mu")) {
  ...
  pooled_estimate <- (diff_compare_summary[["pooled_samples"]] %>% 
  ...
  non_pooled_estimate <- diff_compare_summary[["non_pooled_samples"]] %>% 
    filter(str_detect(parameter, par)) %>%
    mutate(series = str_extract(parameter, "\\d+") %>%
             as.numeric(), model="Kappa parameterization")
