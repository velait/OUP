...
model {
  ...
  for(i in 1:N) {
    ...
    real sigma = sqrt(2*kappa[i] .* lambda[i]);
    
    Y[i] ~ multi_student_t(student_df, vectorize(mu[i], T), ((student_df-2)/student_df)*shape(student_df, sigma, lambda[i], time, T));
  }
