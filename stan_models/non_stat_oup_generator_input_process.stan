functions {
  ...
  matrix gen_oup_cov(real[] x, vector sigma, vector rho){
    ...
    for(i in 1:T) {
      for(j in 1:T) {
        ...
        real sqrt_Q_ij = diff*sqrt(2/(rho[i] + rho[j]));
        
        covariance[i, j] = sigma[i]*sigma[j]*exp(-sqrt_Q_ij);
