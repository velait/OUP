...
functions {
   ...
   matrix matern_3_5_cov(real[] x, real sigma, real rho){
    ...
    for(i in 1:T) {
      for(j in 1:T) {
        real diff = fabs(x[i]-x[j]);
        real term = (sqrt(7)*diff)/rho;
        covariance[i, j] = square(sigma)*(1 + term + 14*square(diff)/(5*square(rho)) + 7*sqrt(7)*pow(diff, 3)/(15*pow(rho, 3)))*exp(-term);
      }
    }
  ...
  matrix matern_3_5_cov2(real[] x1, real[] x2, real sigma, real rho){
  ...
  vector gp_pred_rng(real[] x_pred,
    ...
    N = rows(y_is);
    {
      ...
      matrix[N, N] Sigma;
      Sigma = matern_3_5_cov(x_is, sigma, rho);
      for (n in 1:N)
      ...
      K_div_y_is = mdivide_right_tri_low(K_div_y_is',L_Sigma)';
                                         k_x_is_x_pred = matern_3_5_cov2(x_is, x_pred, sigma, rho);
                                         f_pred_mu = (k_x_is_x_pred' * K_div_y_is);
      v_pred = mdivide_left_tri_low(L_Sigma, k_x_is_x_pred);
      cov_f_pred = matern_3_5_cov(x_pred, sigma, rho) - v_pred' * v_pred;
