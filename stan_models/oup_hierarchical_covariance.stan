...
matrix covariance(real kappa, real inv_lambda, real[] time, int T){
matrix[T, T] covar;
...
model {
  ...
  for(i in 1:N_series) {
      Y[i] ~ multi_normal(rep_vector(mu[i], N_times),
                          covariance(kappa[i], inv_lambda[i], time, N_times) + diag_matrix(rep_vector(square(epsilon), N_times)));
