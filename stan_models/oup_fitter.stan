...
functions {
  ...
  matrix oup_cov(real[] x, real sigma, real rho){
    ...
    for(i in 1:T) {
      for(j in 1:T) {
        covariance[i, j] = square(sigma)*exp(-fabs(x[i]-x[j])/rho);
      }
    }
  ...
  matrix oup_cov2(real[] x1, real[] x2, real sigma, real rho){
    ...
    for(i in 1:T1) {
      for(j in 1:T2) {
        covariance[i, j] = square(sigma)*exp(-fabs(x1[i]-x2[j])/rho);
      }
    }
  ...
  vector gp_pred_rng(real[] x_pred,
    ...
    N = rows(y_is);
    {
      ...
      K_div_y_is = mdivide_right_tri_low(K_div_y_is',L_Sigma)';
        k_x_is_x_pred = oup_cov2(x_is, x_pred, sigma, rho);
      f_pred_mu = (k_x_is_x_pred' * K_div_y_is);
      v_pred = mdivide_left_tri_low(L_Sigma, k_x_is_x_pred);
      cov_f_pred = oup_cov(x_pred, sigma, rho) - v_pred' * v_pred;
