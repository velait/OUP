...
set.seed(1)
gp_set <- generate_gp_set(N_series = N, times, covariance = "oup",
...
oup_sep_res <- lapply(1:N, function(i) {
  ...
  res_df <- lapply(c("length_scale", "stat_var", "epsilon", "sigma"), function(par) {
    ...
    return(rbind(res, res_with_prior) %>% mutate(parameter = par))
...
hier_res <- rbind(oup_hier_with_prior_res, oup_hier_res) %>% mutate(model = "hierarchical")
...
geom_errorbar(data = oup_sep_res, aes(x = index, ymin = lower2.5, ymax = upper97.5, color = model), position ="dodge") +
  ...
  facet_grid(prior ~ parameter, scales = "free", labeller = labeller(.rows =label_both)) +
  labs(subtitle = "Empirical Bayes; time points: 1:15")
