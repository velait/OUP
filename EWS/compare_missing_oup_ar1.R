...
# Randomly remove p% of observations
na_prop <- 25/100
na_index <- sample(1:nrow(ews_residuals), na_prop*nrow(ews_residuals), replace = FALSE)
obs_index <- which(!(1:nrow(ews_residuals) %in% na_index))
...
# CDS set
ar1_ews_res <- ar1_missing_ews_set(ews_residuals[obs_index, ], na_index, obs_index,  iter, chains)
oup_ews_res <- oup_missing_ews_set(ews_residuals[obs_index, ], na_index, obs_index, iter, chains)
generic_ews_res <- generic_ews_set(ews_residuals[obs_index, ])
