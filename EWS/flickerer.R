...
ews_generator <- function(y0, grid, seed = NULL, milstein = FALSE, restrict_pos = FALSE) {
...
times <- seq(from = 0, to = 20000, by = 0.1)
...
flicker_df <- data.frame(y = log(1 + y), x = times) %>% 
  thin(modulo = 1)
